...
for instance in range(1, len(RCPSP_inst_dict_named) + 1):
    ...
    # Objective Function
    model_SDDT.setObjective(quicksum(
        t * (y[n_inst(instance) - 1, t] - negative_index(y, n_inst(instance) - 1, t - 1)) for t in
        range(df_ES.iloc[n_inst(instance) - 1, instance - 1], LS_i)),
        GRB.MINIMIZE)
    ...
    model_SDDT.addConstrs((quicksum(
        r_i_k_inst(instance).iloc[i, k] * (y[i, t] - negative_index(y, i, t - d_i_inst(instance)[i])) for i in
        range(n_inst(instance))) <=
                           R_k_inst(instance)[k]
    ...
    print("Runtime model SDDT for test instance " + str(sName[instance]) + " in seconds: " + str(
        model_SDDT.runtime) + "s")
    ...
    # Objective Function
    model_SDT.setObjective(quicksum(
        t * (y[n_inst(instance) - 1, t] - negative_index(y, n_inst(instance) - 1, t - 1)) for t in
        range(df_ES.iloc[n_inst(instance) - 1, instance - 1], LS_i)),
        GRB.MINIMIZE)
    ...
    model_SDT.addConstrs((quicksum(t * (y[j, t] - negative_index(y, j, t - 1)) for t in range(0, LS_i)) -
                          quicksum(t * (y[i, t] - negative_index(y, i, t - 1)) for t in range(0, LS_i)) >=
                          d_i_inst(instance)[i] for (i, j) in arcs(instance)),
